...
export const Route = createFileRoute("/trace-small")({
  head: () => ({
    meta: [
      { title: "Trace a–z Small Letters — Melly Kids TV" },
      { name: "description", content: "Trace small letters a to z with fun colors." },
    ],
  }),
  component: TraceSmall,
});
        ...
        <div className="grid grid-cols-5 gap-2">
          {LETTERS.map((l, i) => (
            <button
              ...
              style={{ background: TILE_COLORS[i % TILE_COLORS.length], animationDelay: `${(i % 10) * 0.1}s` }}
              aria-label={`Trace letter ${l} for ${WORDS[l]}`}
            >
              <span style={{ fontSize: "1.7rem", lineHeight: 1 }}>{l}</span>
              <span style={{ fontFamily: "'Fredoka', 'Baloo 2', system-ui, sans-serif", fontSize: "0.65rem", fontWeight: 800, marginTop: 2, color: "#fff", textShadow: "1px 1px 0 rgba(0,0,0,0.25)", letterSpacing: "0.02em" }}>
                for {WORDS[l]}
              </span>
            </button>
          ))}
...
function TraceModal({ letter, word, onClose, onComplete }: { letter: string; word: string; onClose: () => void; onComplete: () => void }) {
  ...
  useEffect(() => { speak(`Let's trace the letter ${letter}`); }, [letter]);
      ...
      <div className="relative w-full max-w-sm overflow-hidden rounded-3xl bg-gradient-to-b from-sky-100 to-emerald-50 shadow-2xl ring-4 ring-white" style={{ height: "min(86vh, 680px)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="absolute inset-x-4 top-20 bottom-40 rounded-2xl bg-white/80 ring-2 ring-white shadow-inner overflow-hidden">
          <div className="absolute inset-0 flex items-center justify-center select-none pointer-events-none">
            <span style={{ fontFamily: "'Fredoka', 'Baloo 2', system-ui, sans-serif", fontWeight: 900, fontSize: "min(70vw, 22rem)", lineHeight: 1, color: "transparent", WebkitTextStroke: "5px #94a3b8", letterSpacing: "-0.05em" }}>{letter}</span>
          </div>
          <div className="absolute inset-0 flex items-center justify-center select-none pointer-events-none">
            <span style={{ fontFamily: "'Fredoka', 'Baloo 2', system-ui, sans-serif", fontWeight: 900, fontSize: "min(70vw, 22rem)", lineHeight: 1, color: "transparent", WebkitTextStroke: "2px #cbd5e1", letterSpacing: "-0.05em" }}>{letter}</span>
          </div>
        ...
        <div className="absolute inset-x-0 bottom-3 z-20 flex items-center justify-center gap-3 px-4">
          ...
          <button onClick={() => speak(`Let's trace the letter ${letter}`)} aria-label="Hear it" className="flex size-12 items-center justify-center rounded-full bg-gradient-to-b from-sky-400 to-sky-600 text-white shadow-[0_4px_0_rgba(0,0,0,0.2)] ring-2 ring-white">
...
function SuccessModal({ letter, praise, onAgain, onNext, onClose }: { letter: string; praise: string; onAgain: () => void; onNext: () => void; onClose: () => void }) {
  useEffect(() => {
    playCelebration();
